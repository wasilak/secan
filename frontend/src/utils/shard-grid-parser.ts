 ...
 */
export function parseNodesWithShards(
  nodes: NodeInfo[],
  shards: ShardInfo[]
): NodeWithShards[] {
  ...
  for (const shard of shards) {
    ...
    if (!nodeName) {
      ...
      console.warn(`Could not find node for shard: ${shard.index}[${shard.shard}] on node ${shard.node}`);
      continue;
    }
 ...
 */
export function getShardsForNodeAndIndex(
  node: NodeWithShards,
  indexName: string
): ShardInfo[] {
 ...
 */
export function indexHasProblems(
  indexName: string,
  shards: ShardInfo[]
): boolean {
 ...
 */
export function groupShardsByIndex(
  shards: ShardInfo[]
 ...
 */
export function countShardsByState(
  shards: ShardInfo[]
): Map<string, number> {
