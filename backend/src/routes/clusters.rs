...
pub async fn proxy_request(
    ...
    tracing::debug!(
        "Proxying {} request to cluster '{}' path '{}'",
        method,
        cluster_id,
        path
    );
    ...
    let cluster = state
        ...
        .await
        .map_err(|e| ClusterErrorResponse {
    ...
    let response = cluster
        .request(method, &path, body.map(|j| j.0))
        .await
        .map_err(|e| ClusterErrorResponse {
    ...
    let body_bytes = response.bytes().await.map_err(|e| ClusterErrorResponse {
